...
def voxel_filter(pcd, feature, voxel_size: float, coord_reduction: str = "average"):
    ...
    raveled_idx = torch.tensor(np.ravel_multi_index(vox_idx.T.cpu().numpy(), shape.cpu().numpy()), device=device,
                               dtype=vox_idx.dtype)

    n_pts_per_vox = torch_scatter.scatter(torch.ones_like(raveled_idx, device=device), raveled_idx,
                                          dim_size=shape[0] * shape[1] * shape[2])
    nonzero_vox = n_pts_per_vox.nonzero()
    n_pts_per_vox = n_pts_per_vox[nonzero_vox].squeeze(-1)

    color_vox = \
        torch_scatter.scatter(feature, raveled_idx.unsqueeze(-1), dim=-2, dim_size=shape[0] * shape[1] * shape[2])[
    ...
    if coord_reduction == "center":
        ...
        coord_vox = \
            torch_scatter.scatter(pcd, raveled_idx.unsqueeze(-1), dim=-2, dim_size=shape[0] * shape[1] * shape[2])[
                nonzero_vox].squeeze(-2)
...
def build_graph(xyz, feature, dist_threshold=0.02, self_connection=True, voxelize=False, voxel_size=0.001,
                fiber_in=Fiber({0: 3})):
    ...
    for pcd_index in range(bs):
        ...
        edge_src, edge_dst = radius_graph(current_pcd, dist_threshold * 0.999, max_num_neighbors=max_num_neighbors,
